        ...
        for (out_y = 0; out_y < {{OUTPUT_HEIGHT}}; out_y++)
        {
            ...
            for (out_x = 0; out_x < {{OUTPUT_WIDTH}}; out_x++)
            {
                ...
                for (out_d = 0; out_d < {{OUTPUT_DEPTH}}; out_d++)
                {
                    ...
                    const int f_y_end = min(f_y_start + {{FILTER_HEIGTH}}, {{INPUT_HEIGHT}});

                    float total = 0.f;
                    for (f_y = f_y_start; f_y < f_y_end; f_y++)
                        for (f_x = f_x_start; f_x < f_x_end; f_x++)
                            total += input_data[f_y * {{INPUT_WIDTH}} * {{INPUT_DEPTH}} + f_x * {{INPUT_DEPTH}} + out_d];
                    total /= (f_y_end - f_y_start) * (f_x_end - f_x_start);
                    output_data[f_y * {{OUTPUT_WIDTH}} * {{OUTPUT_DEPTH}} + f_x * {{OUTPUT_DEPTH}} + out_d] = {{call act total}};
                }
            }
        }
